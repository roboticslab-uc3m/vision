...
bool HaarDetection2D::configure(yarp::os::ResourceFinder &rf) {
    ...
    fprintf(stdout,"--------------------------------------------------------------\n");
    if (rf.check("help")) {
        printf("HaarDetection2D options:\n");
    ...
    printf("HaarDetection2D using cropSelector: %d.\n",cropSelector);
    ...
    printf("HaarDetection2D using kinectDevice: %s, kinectLocal: %s, kinectRemote: %s.\n",
        strKinectDevice.c_str(), strKinectLocal.c_str(), strKinectRemote.c_str());
    printf("HaarDetection2D using watchdog: %f.\n",watchdog);
    ...
    if (!rf.check("help")) {
        ...
        printf("[HaarDetection2D] success: kinectDevice available.\n");
        if (! dd.view(kinect) ) fprintf(stderr,"[HaarDetection2D] warning: kinectDevice bad view.\n");
        else printf("[HaarDetection2D] success: kinectDevice ok view.\n");
...
bool HaarDetection2D::updateModule() {
    printf("HaarDetection2D alive...\n");
    return true;
}

/************************************************************************/
bool HaarDetection2D::interruptModule() {
    printf("HaarDetection2D closing...\n");
