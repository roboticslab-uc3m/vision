...
class M3HumanoidShm : public  m3::M3CompShm{
	public:
		...
		right_hand(NULL),right_loadx6(NULL),tmp_cnt(0),left_hand(NULL),left_loadx6(NULL),pwr(NULL),left_gripper(NULL),right_gripper(NULL)
		{		  
	...
	protected:
		...
		M3Joint * right_gripper;
		M3Pwr * pwr;
		M3HumanoidShmSdsCommand command_from_sds;
		M3HumanoidShmSdsStatus status_to_sds;
		int sds_status_size;
		int sds_cmd_size;	
		string bot_name, right_hand_name, right_loadx6_name, left_hand_name, left_loadx6_name, pwr_name, left_gripper_name, right_gripper_name;
		int64_t timeout;
		int tmp_cnt;		
