...
// Needed for ROS
//j//#include <m3ctrl_msgs/M3JointCmd.h>
//j//#include <sensor_msgs/JointState.h>
//j//#include <tf/transform_broadcaster.h>
...
//j//sensor_msgs::JointState joint_state_g;
//m3ctrl_msgs::M3JointCmd joint_cmd_g;
//j//ros::Publisher publisher_g;

//j//ros::Subscriber cmd_sub_g;

//j//ros::Subscriber payload_sub_g;
//j//tf::TransformBroadcaster odom_broadcaster;
////////////////////////////////////////////////////////////////////////////////////


///////  Periodic Control Loop:
void StepShm();
//j//void commandCallback(const m3ctrl_msgs::M3JointCmdConstPtr& msg);
...
//j//    joint_state_g.header.stamp = ros::Time::now();  
    
     int j = 0;
  for (int i = 0; i < ndof_right_arm; i++)
  {
//j//    joint_state_g.position[j] = DEG2RAD(status.right_arm.theta[i]);
//j//    joint_state_g.velocity[j] = DEG2RAD(status.right_arm.thetadot[i]);
//j//    joint_state_g.effort[j] = status.right_arm.torque[i];
    j++;
  ...
  {
//j//    joint_state_g.position[j] = DEG2RAD(status.left_arm.theta[i]);
//j//    joint_state_g.velocity[j] = DEG2RAD(status.left_arm.thetadot[i]);
//j//    joint_state_g.effort[j] = status.left_arm.torque[i];
    j++;  
  }

  for (int i = 0; i < ndof_head; i++)
  {
//j//    joint_state_g.position[j] = DEG2RAD(status.head.theta[i]);
//j//    joint_state_g.velocity[j] = DEG2RAD(status.head.thetadot[i]);
//j//    joint_state_g.effort[j] = status.head.torque[i];
      j++;
  }

  for (int i = 0; i < ndof_torso; i++)
  {
//j//    joint_state_g.position[j] = DEG2RAD(status.torso.theta[i]);
//j//    joint_state_g.velocity[j] = DEG2RAD(status.torso.thetadot[i]);
//j//    joint_state_g.effort[j] = status.torso.torque[i];
      j++;
  }
  
  for (int i = 0; i < ndof_right_hand; i++)
  {
//j//    joint_state_g.position[j] = DEG2RAD(status.right_hand.theta[i]);
//j//    joint_state_g.velocity[j] = DEG2RAD(status.right_hand.thetadot[i]);
//j//    joint_state_g.effort[j] = status.right_hand.torque[i];
      j++;
  }
  
  for (int i = 0; i < ndof_left_hand; i++)
  {
//j//    joint_state_g.position[j] = DEG2RAD(status.left_hand.theta[i]);
//j//    joint_state_g.velocity[j] = DEG2RAD(status.left_hand.thetadot[i]);
//j//    joint_state_g.effort[j] = status.left_hand.torque[i];
      j++;
  }
  
  for (int i = 0; i < ndof_left_gripper; i++)
  {
//j//    joint_state_g.position[j] = DEG2RAD(status.left_gripper.theta[i]);
//j//    joint_state_g.velocity[j] = DEG2RAD(status.left_gripper.thetadot[i]);
//j//    joint_state_g.effort[j] = status.left_gripper.torque[i];
      j++;
  }
  
  
//j//    publisher_g.publish(joint_state_g);
...
//j//	ros::init(argc, argv, "hum_controller", ros::init_options::NoSigintHandler); // initialize ROS node
//j//	ros::AsyncSpinner spinner(1); // Use 1 thread - check if you actually need this for only publishing
//j//	spinner.start();
//j//        ros::NodeHandle root_handle;
//j//	ros::NodeHandle p_nh("~");
	
//j//	cmd_sub_g = root_handle.subscribe<m3ctrl_msgs::M3JointCmd>("humanoid_command", 1, &commandCallback);
	
//j//	joint_state_g = GetInitialJointStateMessage();
//j//	publisher_g = root_handle.advertise<sensor_msgs::JointState>("humanoid_state", 1, true);
