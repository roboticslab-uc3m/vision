...
bool DnnDetector_ParamsParser::getParamValue(const std::string& paramName, std::string& paramValue) const
{
    if (paramName =="trainedModel")
    {
        paramValue = m_trainedModel;
        return true;
    }
    if (paramName =="configDNNModel")
    {
        paramValue = m_configDNNModel;
        return true;
    }
    if (paramName =="framework")
    {
        paramValue = m_framework;
        return true;
    }
    if (paramName =="classesTrainedModel")
    {
        paramValue = m_classesTrainedModel;
        return true;
    }
    if (paramName =="backend")
    {
        paramValue = m_backend;
        return true;
    }
    if (paramName =="target")
    {
    ...
    if (paramName =="scale")
    {
        paramValue = std::to_string(m_scale);
        return true;
    }
    if (paramName =="mean")
    {
        paramValue = std::to_string(m_mean);
        return true;
    }
    if (paramName =="confThreshold")
    {
        paramValue = std::to_string(m_confThreshold);
        return true;
    }
    if (paramName =="nmsThreshold")
    {
        paramValue = std::to_string(m_nmsThreshold);
        return true;
    }
...
std::string DnnDetector_ParamsParser::getConfiguration() const
{
